...
1. **ASHA / ANM / Caregiver Hub**:
  ...
  - Can book hospital appointments on behalf of low-literacy patients using the built-in hospital locator.
2. **Patient Self-Service Route**:
  ...
  - Can chat with an offline AI health assistant, upload lab reports for plain-language summaries, and watch short AI-generated explainer videos on how their prescribed medicines work.
...
- **Nearby Hospital Search**: Powered by the OpenStreetMap API, surfaces hospitals within a 25km radius along with contact numbers for direct emergency outreach.
...
### 4. 📱 Offline AI Health Assistant

Built for the low-connectivity rural context that the rest of the platform targets — this runs on-device, without needing a live internet connection:

- **On-Device Symptom Guidance**: MedGemma (2B-parameter medical language model) runs directly on-device via Google's LiteRT runtime, so patients can get general symptom guidance even with no network access.
- **Lab Report Summarizer**: Patients upload medical reports (e.g., blood test results); MedGemma converts them into a plain-language summary of what the values mean.
- **Medicine Explainer Videos**: The Google Gemini API generates short educational videos explaining how a specific medicine works. These are for general education only — they deliberately exclude dosage or usage recommendations, so they never substitute for a doctor's prescription.
- **Nearby Hospital Locator**: Uses the OpenStreetMap API to find hospitals within a 25km radius with contact details, letting ASHA workers book appointments at the nearest facility on behalf of low-literacy patients.

> **Note:** This offline assistant is a separate subsystem from the cloud-based AI Clinical Decision Support (Section 3 above), which handles vitals triage and safety-clamped clinical rules via the FastAPI/Gemini SDK microservice.

### 5. 📄 Longitudinal EHR & Prescription Safety
...
### 6. 🔐 Cryptographic Ledger & Data Governance
...
| **AI Microservice (Cloud)**   | Python FastAPI / Gemini SDK    | Clinical decision support, OCR extraction, circuit breaker wrapper                   |
| **Offline AI Assistant**      | MedGemma (2B) + Google LiteRT  | On-device symptom guidance and lab report summarization — works without internet     |
| **Medicine Explainer**        | Google Gemini API              | Generates short educational videos on how medicines work                            |
| **Hospital Locator**          | OpenStreetMap API              | Finds hospitals within a 25km radius with contact info, for ASHA worker bookings     |
...
git clone https://github.com/Sumit-Ratna/Swasthya-New.git
cd Swasthya-New/backend
